    ...
    public class UsersFilter
    {
        ...
        public Model.PagingModel<M> Query(IQueryable<M> query)
        {
            ...
            if (!string.IsNullOrWhiteSpace(keyword))
            {
                ...
                data = data.Where(i => EF.Functions.Like(i.FirstName.ToLower(),($"%{keyword}%"))
                                       ||EF.Functions.Like(i.LoginName.ToLower(),($"%{keyword}%"))
                                       ||EF.Functions.Like(i.LastName.ToLower(),($"%{keyword}%")));
            }
